                ...
                <%--    <div>
                    ...
                    <div style="padding-top: 30px;">
                        <table style="width: 700px;border-style:1px solid " border="1">
                            ...
                            <c:forEach items="${thisOrder}" var="item">
                                <tr id="tr${item.product.pid}">
                                    <td><input type="checkbox" class="ids" name="pid" value="${item.product.pid}" checked>
                                    </td>
                                    <td><img src="/${item.product.imgurl}" width="100" height="150"/></td>
                                    <td>${item.product.pname}</td>
                                    <td>${item.buynum}</td>
                                    <input type="hidden"  name="buynum" value="${item.buynum}"/>
                                    <td>${item.product.estoreprice}</td>
                                    <td id="price">${item.buynum * item.product.estoreprice}</td>
                                </tr>
                                <c:set var="sum" value="${buynum+item.buynum*item.product.estoreprice}"> </c:set>
                            </c:forEach>
                        </table>
                    </div>
                    <input type="hidden" id="qq" name="money" value="${buynum}"/>
                  <%--  <h4>总金额:  <span class="sumPrice">${buynum}</span>元</h4>--%>
